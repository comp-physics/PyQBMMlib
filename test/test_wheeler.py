...
def generate_gaussian_moments(mu, sigma):
    ...
    moments[5] = (
        mu ** 5.0 + 10.0 * (mu ** 3.0) * (sigma ** 2.0) + 15.0 * mu * sigma ** 4.0
    )
    moments[6] = (
        mu ** 6.0
        + 15.0 * (mu ** 4.0) * (sigma ** 2.0)
        + 45.0 * (mu ** 2.0) * (sigma ** 4.0)
        + 15.0 * sigma ** 6.0
    )
    moments[7] = (
        mu ** 7.0
        + 21.0 * (mu ** 5.0) * (sigma ** 2.0)
        + 105.0 * (mu ** 3.0) * (sigma ** 4.0)
        + 105.0 * mu * sigma ** 6.0
    )
...
def test_wheeler(test, mu, sigma, qbmm_mgr, tol):
    ...
    if not success:
        print(
            "test_wheeler: Test(%i): Moments = [{:s}]".format(
                ", ".join(["{:.4e}".format(p) for p in moments])
            )
            % test
        )
        ...
        print(
            "\t abscissas = [{:s}]".format(
                ", ".join(["{:.4e}".format(p) for p in g_abs])
            )
        )
        print(
            "\t weights   = [{:s}]".format(
                ", ".join(["{:.4e}".format(p) for p in g_wts])
            )
        )
        ...
        print(
            "\t abscissas = [{:s}]".format(
                ", ".join(["{:.4e}".format(p) for p in my_abs])
            )
        )
        print(
            "\t weights   = [{:s}]".format(
                ", ".join(["{:.4e}".format(p) for p in my_wts])
            )
        )
