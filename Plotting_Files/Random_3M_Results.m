...
load(['../ML_Code/3M_Random_MLQBMM','.mat']);
total_cases = 30;
total_times = 2001;

T = linspace(0,200,total_times);
T_qbmm = linspace(0,200,qbmm_times);

R30_ML = zeros(total_cases,total_times);
R21_ML = zeros(total_cases,total_times);
R12_ML = zeros(total_cases,total_times);
R03_ML = zeros(total_cases,total_times);

R30_MC = zeros(total_cases,total_times);
R21_MC = zeros(total_cases,total_times);
R12_MC = zeros(total_cases,total_times);
R03_MC = zeros(total_cases,total_times);

R30_QBMM = zeros(total_cases,qbmm_times);
R21_QBMM = zeros(total_cases,qbmm_times);
R12_QBMM = zeros(total_cases,qbmm_times);
R03_QBMM = zeros(total_cases,qbmm_times);

Pressure = zeros(total_cases,total_times);

for kk=1:total_cases
   for tt=1:total_times
        R30_MC(kk,tt) = output_data(kk,1,tt);
        R03_MC(kk,tt) = output_data(kk,2,tt);
        R40_MC(kk,tt) = output_data(kk,3,tt);
        R04_MC(kk,tt) = output_data(kk,4,tt);
        
        R30_ML(kk,tt) = predictions(kk,1,tt);
        R03_ML(kk,tt) = predictions(kk,2,tt);
        R40_ML(kk,tt) = predictions(kk,3,tt);
        R04_ML(kk,tt) = predictions(kk,4,tt);
        
        
        Pressure(kk,tt) = input_data(kk,6,tt);
        
   end
   R30_QBMM(kk,:) = qbmm_predictions(kk,1,:);
   R03_QBMM(kk,:) = qbmm_predictions(kk,2,:);
   R40_QBMM(kk,:) = qbmm_predictions(kk,3,:);
   R04_QBMM(kk,:) = qbmm_predictions(kk,4,:);
end
...
for ii=1:total_cases
    for jj=1:4
        for tt=1:total_times-1
            ...
            l2_error_qbmm(jj,ii) = l2_error_qbmm(jj,ii) +(output_data(ii,jj,tt)-qbmm_predictions(ii,jj,10*(tt-1)+1))^2;
        end
...
set(p(iflag),'XTickLabel',[{'\mu_{3,0}','\mu_{0,3}','\mu_{4,0}','\mu_{0,4}'}],'fontsize',7)
if (ii == 1)
...
hold on
plot(T_qbmm,R30_QBMM(plot_cases(ii),:),'Color',[0.8,0,0],'linewidth',1.0,'linestyle','--') 
hold on
...
plot(T(tstart:tend),R03_MC(plot_cases(ii),tstart:tend),'Color',[0.2,0.2,0.2],'linewidth',1.0,'linestyle','-') 
hold on
plot(T_qbmm,R03_QBMM(plot_cases(ii),:),'Color',[0.8,0,0],'linewidth',1.0,'linestyle','--') 
hold on
plot(T,R03_ML(plot_cases(ii),:),'Color',[0,0.5,1],'linewidth',1.0) 
xlim([T(tstart),T(tend)])

ymin = min([floor(20*min(min(0.9*R03_MC(plot_cases(ii),tstart:tend))))/20,floor(20*min(min(0.9*R03_ML(plot_cases(ii),tstart:tend))))/20,floor(20*min(min(0.9*R03_QBMM(plot_cases(ii),tstart:tend))))/20]);
ymax = max([ceil(20*max(max(1.1*R03_MC(plot_cases(ii),tstart:tend))))/20,ceil(20*max(max(1.1*R03_ML(plot_cases(ii),tstart:tend))))/20,ceil(20*max(max(1.1*R03_QBMM(plot_cases(ii),tstart:tend))))/20]);
ylim([ ymin, ymax   ])
...
if (ii == 1)
yname1 = ylabel("$\mu_{0,3}$",'interpreter','latex','fontsize',10);
set(p(iflag),'YLabel',yname1);
ylabh = get(gca,'ylabel');
set(ylabh,'position',get(ylabh,'position') - [-0.8 0 0]);
end
...
plot(T(tstart:tend),R40_MC(plot_cases(ii),tstart:tend),'Color',[0.2,0.2,0.2],'linewidth',1.0,'linestyle','-') 
hold on
plot(T_qbmm,R40_QBMM(plot_cases(ii),:),'Color',[0.8,0,0],'linewidth',1.0,'linestyle','--') 
hold on
plot(T,R40_ML(plot_cases(ii),:),'Color',[0,0.5,1],'linewidth',1.0) 
xlim([T(tstart),T(tend)])

ymin = min([floor(100*min(min(0.9*R40_MC(plot_cases(ii),tstart:tend))))/100,floor(100*min(min(0.9*R40_ML(plot_cases(ii),tstart:tend))))/100,floor(100*min(min(0.9*R40_QBMM(plot_cases(ii),tstart:tend))))/100]);
ymax = max([ceil(100*max(max(1.1*R40_MC(plot_cases(ii),tstart:tend))))/100,ceil(100*max(max(1.1*R40_ML(plot_cases(ii),tstart:tend))))/100,ceil(100*max(max(1.1*R40_QBMM(plot_cases(ii),tstart:tend))))/100]);
ylim([ ymin, ymax   ])
...
if (ii == 1)
yname1 = ylabel("$\mu_{4,0}$",'interpreter','latex','fontsize',10);
set(p(iflag),'YLabel',yname1);
ylabh = get(gca,'ylabel');
set(ylabh,'position',get(ylabh,'position') - [-0.8 0 0]);
end
...
plot(T(tstart:tend),R04_MC(plot_cases(ii),tstart:tend),'Color',[0.2,0.2,0.2],'linewidth',1.0,'linestyle','-') 
hold on
plot(T_qbmm,R04_QBMM(plot_cases(ii),:),'Color',[0.8,0,0],'linewidth',1.0,'linestyle','--') 
hold on
plot(T,R04_ML(plot_cases(ii),:),'Color',[0,0.5,1],'linewidth',1.0) 
xlim([T(tstart),T(tend)])
ymin = min([floor(20*min(min(0.9*R04_MC(plot_cases(ii),tstart:tend))))/20,floor(20*min(min(0.9*R04_ML(plot_cases(ii),tstart:tend))))/20,floor(20*min(min(0.9*R04_QBMM(plot_cases(ii),tstart:tend))))/20]);
ymax = max([ceil(20*max(max(1.1*R04_MC(plot_cases(ii),tstart:tend))))/20,ceil(20*max(max(1.1*R04_ML(plot_cases(ii),tstart:tend))))/20,ceil(20*max(max(1.1*R04_QBMM(plot_cases(ii),tstart:tend))))/20]);
ylim([ ymin, ymax   ])
...
if (ii == 1)
yname1 = ylabel("$\mu_{0,4}$",'interpreter','latex','fontsize',10);
set(p(iflag),'YLabel',yname1);
ylabh = get(gca,'ylabel');
set(ylabh,'position',get(ylabh,'position') - [-0.8 0 0]);
...
print(figeta,'-dpdf','3M_Random_Forcing_Results.pdf');
savefig(figeta,'3M_Random_Forcing_Results.fig');