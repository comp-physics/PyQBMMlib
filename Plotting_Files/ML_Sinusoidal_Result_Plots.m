...
load(['../ML_Code/HM_Sinusoidal_MLQBMM','.mat']);   
...
mc_data = zeros(total_cases,4,total_times);
qbmm_data = zeros(total_cases,4,total_times);

output_vals = [6,7,17,21]; % mu30, mu21, mu32, mu3(1-gamma)0
input_vals  = [1,2,3,4,5,6,7,17,21];
...
for jj=1:total_cases      
    ...
    for ii=1:4
        ...
        qbmm_data(jj,ii,:) = input_data(jj,ii+5,:);
...
for jj=1:total_cases
    R30_MC(jj,:) = mc_data(jj,1,:);
    R21_MC(jj,:) = mc_data(jj,2,:);
    R32_MC(jj,:) = mc_data(jj,3,:);
    R3g_MC(jj,:) = mc_data(jj,4,:);
    
    R30_ML(jj,:) = predictions(jj,1,:);
    R21_ML(jj,:) = predictions(jj,2,:);
    R32_ML(jj,:) = predictions(jj,3,:);
    R3g_ML(jj,:) = predictions(jj,4,:);
    
    R30_QBMM(jj,:) = qbmm_data(jj,1,:);
    R21_QBMM(jj,:) = qbmm_data(jj,2,:);
    R32_QBMM(jj,:) = qbmm_data(jj,3,:);
