...
file_name_local = 'build/results.csv'
...
def plot_result(data):
    ...
    ax[0].plot(x, data[1:, 1], label='OMP 1 core', color='b')
...
def plot_compared_result(data1, data2):
    ...
    plt.plot(x, data1[1:, 1], label='fftw_local', color='b')
    # plt.plot(x, data1[1:, 2], label='cufft sequential_local', color='r')
    ...
    plt.xlabel('Input size (N of 64x64x64)')
    plt.ylabel('Time (s)')
    plt.legend()
...
if __name__ == "__main__":
    ...
    plot_result(data1)
    # plot_compared_result(data1, data2)
